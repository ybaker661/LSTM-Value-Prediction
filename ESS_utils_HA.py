...
def CalcValueNoUnc(d, c, P, eta, vi, ed, iC, iD):
    ...
    #alternate way to calculate terms, both this and the above work but the above is cleaner
    # FtEC = (vi*eta > d).astype(int) # F_t(v_t(e)*eta)
    # FtCC = (vC*eta > d).astype(int) # F_t(v_t(e+P*eta)*eta)
    # FtED = ((vi/eta + c)*((vi/eta + c) > 0) > d).astype(int) # F_t(v_t(e)/eta + c) 
    # FtDD = ((vD/eta + c)*((vD/eta + c) > 0) > d).astype(int) # F_t(v_t(e-P/eta)/eta + c) 
...
def ArbValue(lmp, v, e, P, E, eta, c, N):
    ...
    # 1 segment arbitrage adapted directly from arbval_nosoc in matlab
    if(N==1):
        vC = v*eta
        vD = v/eta+c
        pC = P*(vC > lmp) 
...
def generate_train_CNN_RTP(T, tlambda, start, stop, lastDay, num_RTP, vAvg, step = int((5*12))):


  #generates training set by ex
...
def CNNLSTM_evaluate(model, DAP, RTP, 
  ...
  eta=0.9, c=10, rebin=None):
    ...
    for t in range(T2): # start from the first day and move forwards
        vv = v3[:, t+1]
        e, p = ArbValue(tlambda_RTP_test[day+t+hr], vv, e, P, 1, eta, c, v3.shape[0])
        eS_test[t] = e # record SoC
        pS_test[t] = p # record Power


    ProfitOutTest = np.sum(pS_test * tlambda_RTP_test[day+hr:v3.shape[1]+day+hr]) - np.sum(c * pS_test[pS_test>0])
    RevenueTest = np.sum(pS_test * tlambda_RTP_test[day+hr:v3.shape[1]+day+hr])
    # for t in range(T2): # start from the first day and move forwards
    #     vv = v3[:, t+1]
...
#this function is just for the cell that visualizes training
#generates prediction for one year as you would in arbitrage, without arbitraging
def predictor(model, DAP, RTP, 
  startTest, stopTest, lastDay, 
  num_DAP, num_RTP, Pr=0.25, Ts=1/12,
  eta=0.9, c=10):
...
def CNNLSTM_evaluate_RTP(model,RTP, 
  startTest, stopTest, lastDay, 
  num_RTP, Q=False, Pr=0.25,
  eta=0.9, c=10,
  rebin = None):
    ...
    if Q:
      tlambda_RTP_test = RTP.flatten('F')
      T2 = int(tlambda_RTP_test.shape[0] - (24/Ts)) -288 -12
      # print('Queensland Timesteps considered: ', T2)
      # print('RTP length: ', tlambda_RTP_test.shape)
    else:
      tlambda_RTP_test = RTP[:, (len(RTP[0])-lastDay+startTest-2):(len(RTP[0])-lastDay+stopTest+1)]
      tlambda_RTP_test = tlambda_RTP_test.flatten('F')

    X_test = np.zeros((T2, num_RTP))

    for t in range(T2):
      X_test[t, :] = tlambda_RTP_test[t+289-num_RTP : t+289]
    ...
    tstart = time.time()
    v3 = model.predict(x, verbose=0)
    # print("network prediction shape: ", v3.shape)
    # print('time to predict full year:', time.time()-tstart)
    v3 = np.asarray(v3)
    v3 = np.reshape(v3, (int(v3.shape[0]*v3.shape[1]), v3.shape[2]))
    # print('value func has shape:', v3.shape)
    ...
    if Q:
      T2 = v3.shape[1] -1
      eS_test = np.zeros(v3.shape[1]-1) # generate the SoC series
      pS_test = np.zeros(v3.shape[1]-1) # generate the power series
    else:
      T2 = v3.shape[1] 
      eS_test = np.zeros(v3.shape[1]) # generate the SoC series
      pS_test = np.zeros(v3.shape[1]) # generate the power series
    ...
    for t in range(T2): # start from the first day and move forwards
        vv = v3[:, t+1]
        e, p = ArbValue(tlambda_RTP_test[288+t+12], vv, e, P, 1, eta, c, v3.shape[0])
        eS_test[t] = e # record SoC
        pS_test[t] = p # record Power

    if Q:
      ProfitOutTest = np.sum(pS_test * tlambda_RTP_test[288+12:T2+288+12]) - np.sum(c * pS_test[pS_test>0])
      RevenueTest = np.sum(pS_test * tlambda_RTP_test[288+12:T2+288+12])

    else:

      ProfitOutTest = np.sum(pS_test * tlambda_RTP_test[288+12:v3.shape[1]+288+12]) - np.sum(c * pS_test[pS_test>0])
      RevenueTest = np.sum(pS_test * tlambda_RTP_test[288+12:v3.shape[1]+288+12])
...
#THESE FUNCTIONS ARE FOR 5 MIN ARBITRAGE ON 1HR VALUATION
